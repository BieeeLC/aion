<script>
	...
	$('document').ready(function(){
		$('i.btn-point-gold').click(function(){
			...
			if (chc == '0') $("#webshop-count").html('<span class="disabled-count">Количество: '+cnt+'</span>');
		});
			...
			<?php $form=$this->beginWidget('CActiveForm', array('id'=>'form-buy')); ?>
				<div class="table">
					...
					<div class="row">
						...
						<div>
							<?php echo CHtml::ajaxSubmitButton('Купить', Power::url('webshop/buy'),
							array('beforeSend'=>'function(){$("#webshopBuyButton").attr({disabled: true});}', 'success'=>'function(html){$("#webshopBuyButton").attr({disabled: false}); $(".note-result").html(html);}'),
							array('live'=>true, 'class'=>'button', 'id'=>'webshopBuyButton')); ?>
						</div>
					</div>
				</div>
			<?php $this->endWidget(); ?>
